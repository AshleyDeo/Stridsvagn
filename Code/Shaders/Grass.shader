Shader "Custom Shaders/Grass" {
    Properties {
        ...
        _BladeTex ("Blade Texture", 2D) = "white" {}
		...
		_BladeBendCurve("Blade Curvature Amount", Range(1, 4)) = 3

		_BendDelta("Blade Variation", Range(0, 1)) = 0.2
		...
		_WindFreq("Wind Pulse Frequency", Range(0, 1)) = 0.01
    ...
    SubShader {
		...
		HLSLINCLUDE
			...
			VertexInput hull(InputPatch<VertexInput, 3> patch, uint id : SV_OutputControlPointID) { return patch[id]; }
			...
			GeomData TransformGeomToClip(float3 pos, float3 offset, float3x3 tranformationMatrix, float2 uv) {
				GeomData o;
				o.pos = TransformObjectToHClip(pos + mul(tranformationMatrix, offset));
				o.uv = uv;
				o.worldPos = TransformObjectToWorld(pos + mul(tranformationMatrix, offset));
				return o;
			}
			...
			void geom(point VertexOutput input[1], inout TriangleStream<GeomData> triStream) {
				...
				if (grassVisibility >= _GrassThreshold) {
					...
					//Wind Movement
					float2 windUV = pos.xz * _WindMap_ST.xy + _WindMap_ST.zw + normalize(_WindVelocity.xzy) * _WindFreq * _Time.y;
		...
		Pass { //Draws Grass
			...
			HLSLPROGRAM
				...
				float4 frag(GeomData i) : SV_Target {
					float4 color = tex2D(_BladeTex, i.uv);
